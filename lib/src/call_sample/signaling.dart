...
class Signaling {
  ...
  var description; //just to make values global
  var data; //just to make values global
      ...
      case 'offer':
        {
          ...
          /*    ===> Move the code block here to an accpet method <===
  ...
  void callPicked() {
    _createPeerConnection(data['from'], data['media'], false).then((pc) {
      ///from here
      _peerConnections[data['from']] = pc;
      pc.setRemoteDescription(
          new RTCSessionDescription(description['sdp'], description['type']));

      ///to here want to run inside the switch case

//this part below connects even after 5secs delay still connect
      Future.delayed(const Duration(milliseconds: 5000), () {
        if (this.onStateChange != null) {
          this.onStateChange(SignalingState.CallStateNew);
        }
        _createAnswer(data['from'], pc, data['media']);
      });
    });
  }
